...
summary: "A detailed step-by-step tutorial on how to connect Apache Kafka to a Spring Integration Channel using Spring Kafka and Spring Boot."
url: /spring-kafka-spring-integration-example.html
date: 2017-05-13
lastmod: 2017-05-13
...
The [Spring Integration Kafka](https://github.com/spring-projects/spring-integration-kafka) extension project provides inbound and outbound channel adapters specifically for [Apache Kafka](https://kafka.apache.org/).
...
The building of this project will be automated using [Maven](https://maven.apache.org/). We include the needed Spring Integration dependencies using the `spring-boot-starter-integration` [Spring Boot starter](https://github.com/spring-projects/spring-boot/tree/master/spring-boot-project/spring-boot-starters). For testing support, we also include the `spring-boot-starter-test` starter.

As we will be using the Spring Integration Kafka extension, we add the corresponding `spring-integration-kafka` dependency. Starting from version 2.0 this project is a complete rewrite based on the [Spring for Apache Kafka](https://projects.spring.io/spring-kafka/) project which uses the pure java `Producer` and `Consumer` clients provided by Kafka. As such we also add the `spring-kafka` dependency for core functionality as well as `spring-kafka-test` in order to have access to an embedded Kafka broker when running our unit test.
...
We create a `SpringKafkaIntegrationApplication` class that takes care of some [basic Spring Boot setup](http://docs.spring.io/autorepo/docs/spring-boot/current/reference/html/using-boot-using-springbootapplication-annotation.html) and which also allows launching the application.
...
A **Message Endpoint** represents the "filter" of a [pipes-and-filters architecture](http://www.enterpriseintegrationpatterns.com/patterns/messaging/PipesAndFilters.html). Spring Integration has a number of endpoint types that are supported. In this example, we will look at the endpoint types that allow us to connect to Kafka.
...
![spring kafka spring integration example](spring-kafka-spring-integration-example.png)

Our example will consist out of two channels as shown in above diagram. The first _ProducingChannel_ will have a `kafkaMessageHandler` that subscribes to the channel and writes all received messages to a `spring-integration-kafka.t` topic.
...
The sent messages should be picked up by the _ConsumingChannel_ bean and when passed to the `CountDownLatchHandler` the `CountDownLatch` will be lowered from its initial value `10`. We then check if the 10 messages have been receive by asserting that the `CountDownLatch` value is equals to `0`.
...
{{< alert "github" >}}
If you would like to run the above code sample you can get the [full source code on GitHub](https://github.com/code-not-found/spring-kafka/tree/master/spring-kafka-spring-integration-helloworld).
{{< /alert >}}
